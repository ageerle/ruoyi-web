...
import {
  ...
  NModal, NDivider, NIcon, useThemeVars
} from "naive-ui";
...
// 微信登录相关
const wxLoginModal = ref(false);
const qrCode = ref("");
const ticket = ref("");
const wxLoginLoading = ref(false);
const wxLoginError = ref(false);
const wxLoginErrorMsg = ref("");

let qrCodeCheckInterval: NodeJS.Timer | undefined;

async function handleWxLogin() {
  try {
    wxLoginModal.value = true;
    wxLoginLoading.value = true;
    wxLoginError.value = false;

    const res = await getMpQrCode();
    qrCode.value = res.data.qrCodeUrl;
    ticket.value = res.data.ticket;

    // 开始轮询检查登录状态
    qrCodeCheckInterval = setInterval(checkWxLoginStatus, 3000);
  } catch (error: any) {
    wxLoginError.value = true;
    wxLoginErrorMsg.value = error.message || t("login.qrCodeFailed");
    message.error(wxLoginErrorMsg.value);
  } finally {
    wxLoginLoading.value = false;
  }
}

async function checkWxLoginStatus() {
  try {
    const res = await getLoginType(ticket.value);
    if (res.data.token) {
      await userStore.userQrLogin(res.data.token);
      clearInterval(qrCodeCheckInterval);
      wxLoginModal.value = false;
      message.success(t("login.loginSuccess"));
      router.push("/");
    }
  } catch (error: any) {
    ...
    clearInterval(qrCodeCheckInterval);
    wxLoginError.value = true;
    wxLoginErrorMsg.value = error.message || "登录状态检查失败，请重试";
  }
}

// 重试获取二维码
function retryWxLogin() {
  handleWxLogin();
}

// 关闭微信登录弹窗
function closeWxLoginModal() {
  wxLoginModal.value = false;
        ...
        <div class="form-wrapper">
          ...
          <div class="form-content">
            ...
            <div class="alternative-logins">
              <NButton circle secondary @click="handleWxLogin" class="wx-login-button" title="微信登录">
                <NIcon :component="LogoWechat" size="22" />
              </NButton>
            </div>
